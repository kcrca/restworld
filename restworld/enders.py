...
from pyker.commands import mc, r, entity, WEST, MOVE, self, NORTH, EAST, SOUTH
...
def room():
    ...
    def cage_loop(step):
        if step.i == 0:
            yield mc.execute().at(entity().tag('crystal_home')).run().fill(r(2, 7, 2), r(-2, 4, -2), 'air').replace(
                'iron_bars')
        else:
            yield mc.execute().at(entity().tag('crystal_home')).run().fill(
                r(2, 9, 2), r(-2, 6, -2), 'iron_bars').hollow()
            yield mc.execute().at(entity().tag('crystal_home')).run().fill(r(1, 8, 1), r(-1, 6, -1), 'air')
            yield mc.execute().at(entity().tag('crystal_home')).run().clone(
    ...
    room.function('crystal_init').add(
        label(r(4, 2, 0), 'Cage'),
        mc.execute().as_(entity().tag('var_home')).run().tag(self()).add('blockers_home'))
    ...
    def crystal_loop(step):
        if step.i == 0:
            ...
            yield mc.kill(entity().tag('crystal'))

    room.loop('crystal', main_clock).add(mc.kill(entity().tag('crystal'))).loop(crystal_loop, range(0, 2))
    ...
    room.function('dragon_init').add(
        mc.kill(entity().type('ender_dragon')),
        mc.kill(entity().type('dragon_fireball')),
        ...
        room.mob_placer(r(0, 3, -14), NORTH, adults=True).summon('dragon_fireball', tags=('dragon_thing',),
                                                    nbt={'direction': {0.0, 0.0, 0.0}, 'ExplosionPower': 0}),
    )
    ...
    room.function('enderman_enter').add(
        mc.execute().unless().entity(entity().type('enderman').distance((None, 5))).run(
    ...
    room.loop('shulker_color', main_clock).loop(
        lambda step: mc.data().merge(entity().tag('shulker').limit(1), {'Color': step.i}), colors)
    ...
    def shulker_loop(step):
        yield mc.data().merge(entity().tag('shulker').limit(1), {'Peek': step.elem})
        yield mc.data().modify(entity().tag('shulker').limit(1), 'Rotation').set().value([0, 0])
