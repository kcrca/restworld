...
from pyker.commands import NORTH, r, WEST, EAST, mc, entity, self, Entity, good_facing
...
def room():
    ...
    room.loop('creeper', main_clock).loop(
        lambda step: mc.execute().as_(entity().tag('creeper').limit(1)).run().data().merge(
            self(), {'powered': step.elem}), (True, False))
    ...
    room.loop('slime', main_clock).loop(
        lambda step: mc.data().modify(entity().tag('slime').limit(1), 'Size').set().value(step.elem),
    ...
    room.loop('illager', main_clock).add(kill_em(entity().tag(*tags))).loop(illager_loop, illagers)
    ...
    room.loop('ravager', main_clock).add(kill_em(entity().tag('ravager'))).loop(
    ...
    room.loop('skeleton_horse').add(
        kill_em(entity().tag('skeleton_horse', '!kid'))
    ...
    room.function('zombie_init').add(
        zombie_jockey.set(0),
        mc.execute().as_(entity().tag('zombie_home')).run().tag(self()).add('zombie_home_selector'),
        mc.execute().as_(entity().tag('zombie_jockey_home')).run().tag(self()).add('zombie_home_selector'),
    ...
    def zombie_loop(step):
        ...
        if step.elem == 'Drowned':
            yield mc.execute().as_(entity().tag('zombieish').tag('!kid')).run().data().merge(self(), {
                'HandItems': [Item.nbt_for('trident')]})

    room.loop('zombie', main_clock).add(kill_em(entity().tag('zombieish'))).loop(
