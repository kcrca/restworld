...
stand_tmpl = Entity('armor_stand', {
    'Invisible': True, 'NoGravity': True, 'ShowArms': True, 'Pose': {'LeftArm': [0, 90, 90]}, 'Tags': ['banner_stand']})
...
def room():
    ...
    def init_a_stand(x, xn, z, zn, angle, facing, bx, bz, y_banner, y_shield, pattern, handback=None):
        ...
        stand.merge_nbt({'Rotation': [angle, 0], 'equipment': {'offhand': shield.nbt}}).tag('banners',
                                                                                            'banner_shield_stand')
    ...
    update = room.function('update_banners', home=False).add(
        execute().as_(stands).run(
            ...
            execute().if_().data(s(), 'equipment.offhand.components.banner_patterns').run(
                data().modify(s(), 'equipment.offhand.components.banner_patterns[].color').set().value(
                    Arg('ink')))),
