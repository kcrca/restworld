...
def color_functions(room):
    ...
    riders = tuple(room.riders_on(n().tag('colorings_ghast'), tags=f'ghast_rider_{i}') for i in range(4))
    room.function('ghast_riders_on', home=False).add(riders)
    room.function('ghast_riders_off', home=False).add((room.riders_off(),) * 4)
