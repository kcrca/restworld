...
def room():
    ...
    moon_init.add(
        ...
        summon(('item_frame',
                {'Facing': as_facing(EAST).number, 'Item': Item.nbt_for('clock'),
                 'Tags': ['time_frame', room.name], 'Fixed': True}),
               r(-10, 8, 4)),
        label(r(-2, 7, 4), 'Moon Phases', EAST),
        label(r(-1, 7, 4), 'eReset Room', EAST, vertical=True),
        label(r(-9, 7, 4), 'wReset Room', WEST, vertical=True),
        label((22, 106, 31), 'nReset Room', NORTH, vertical=True, tags=('vn')),
        label((22, 106, 31), 'sReset Room', SOUTH, vertical=True, tags=('vs')),
    )
    ...
    room.function('run_time_init').add(
        ...
        label(r(0, 7, 1), 'Time Running', WEST),
        label(r(0, 7, -1), 'Time Direction', WEST),
        label(r(0, 7, -4), 'Reset Room', NORTH),
    )
    room.function('worldborder_hide').add(
        worldborder().center((0, 0)),
        worldborder().set(59999968))
    room.function('worldborder_init').add(label(r(0, 7, 0), 'World Boundary', EAST))
