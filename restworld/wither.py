...
from pyker.commands import NORTH, r, WEST, mc, entity, good_facing, EAST
...
def room():
    ...
    room.function('painting_init').add(
        mc.kill(entity().type('painting').distance((None, 10))),
    ...
    room.function('wither_mob_exit').add(kill_em(entity().tag('wither_mob')))

    def wither_loop(step):
        if step.i == 0:
            yield mc.data().merge(entity().tag('wither_mob').limit(1), {'Health': 300, 'Invul': 100})
            yield mc.data().merge(r(0, 2, -2), {'Text2': 'Invulnerable (New)'})
        elif step.i == 1:
            yield mc.data().merge(entity().tag('wither_mob').limit(1), {'Health': 300, 'Invul': 0})
            yield mc.data().merge(r(0, 2, -2), {'Text2': 'Healthy'})
        else:
            yield mc.data().merge(entity().tag('wither_mob').limit(1), {'Health': 140, 'Invul': 0})
    ...
    room.function('wither_skull_init').add(
        mc.kill(entity().type('wither_skull')),
