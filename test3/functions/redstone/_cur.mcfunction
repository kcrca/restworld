execute at @e[tag=daylight_sensor_home] run function v3:redstone/daylight_sensor_cur
execute at @e[tag=redstone_torch_home] run function v3:redstone/redstone_torch_cur
execute at @e[tag=repeater_home] run function v3:redstone/repeater_cur
execute at @e[tag=piston_home] run function v3:redstone/piston_cur
execute at @e[tag=observer_home] run function v3:redstone/observer_cur
execute at @e[tag=redstone_dust_home] run function v3:redstone/redstone_dust_cur
execute at @e[tag=redstone_lamp_home] run function v3:redstone/redstone_lamp_cur
execute at @e[tag=rail_home] run function v3:redstone/rail_cur
execute at @e[tag=hopper_home] run function v3:redstone/hopper_cur
execute at @e[tag=wood_power_home] run function v3:redstone/wood_power_cur
execute at @e[tag=note_block_home] run function v3:redstone/note_block_cur
execute at @e[tag=redstone_block_home] run function v3:redstone/redstone_block_cur
execute at @e[tag=pressure_plate_home] run function v3:redstone/pressure_plate_cur
execute at @e[tag=minecarts_home] run function v3:redstone/minecarts_cur
execute at @e[tag=dispenser_home] run function v3:redstone/dispenser_cur
execute at @e[tag=lever_home] run function v3:redstone/lever_cur
execute at @e[tag=finish_home] run setblock ~1 ~ ~ minecraft:redstone_block
