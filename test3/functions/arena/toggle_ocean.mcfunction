...
execute if score arena_is_ocean funcs matches 0 at @e[tag=controls_home] run data merge block ~-2 ~1 ~-1 {Text4:"\"Off\""}
execute if score arena_is_ocean funcs matches 1 run fill ~-20 ~0 ~-20 ~20 ~16 ~20 air replace water
execute if score arena_is_ocean funcs matches 1 at @e[tag=controls_home] run data merge block ~-2 ~1 ~-1 {Text4:"\"On\""}
